...
export const CategoryProvider = ({ children }: CategoryProviderProps) => {
  ...
  useEffect(() => {
    const fetchConditionCategories = async () => {
      try {
        setIsLoading(true);
        const response = await api.get("/stp_operation/get_suitability_by_category?category=condition&all_data=true")
    ...
    const fetchConstraintCategories = async () => {
      try {
        const response = await api.get("/stp_operation/get_suitability_by_category?category=constraint&all_data=true") 
        if (response.status != 201) {
          throw new Error('Failed to fetch condition categories');
        }
        const data = await response.message as Category[];
    ...
    const fetchArea = async ()=>{
      try{
        const response = await api.get("/stp_operation/get_stp_suitability_area") 
