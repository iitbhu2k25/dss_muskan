...
interface SubIndustryCount {
  [subIndustryName: string]: number;
}

interface IndustrialSelection {
  [industryType: string]: {
    selected: boolean;
    subIndustries: {
...
interface DemandContextType {
  ...
  // Industrial State
  industrialSelection: IndustrialSelection;
  ...
  // Industrial Actions
  toggleIndustryType: (industryType: string) => void;
  toggleSubIndustry: (industryType: string, subIndustryName: string) => void;
  updateSubIndustryCount: (industryType: string, subIndustryName: string, count: number) => void;
...
export const DemandProvider: React.FC<DemandProviderProps> = ({ children }) => {
  ...
  // Industrial State - Initialize with structure including SWC values
  const [industrialSelection, setIndustrialSelection] = useState<IndustrialSelection>({
    "Thermal Power Plants": {
      selected: false,
      subIndustries: {
        "Small (Up to 1000 MW)": { selected: false, count: 0, swc: "2.3 - 3.9" },
        "Medium (Between 1000 to 2500 MW)": { selected: false, count: 0, swc: "1.9 - 6.5" },
        "Large (2500 MW and more)": { selected: false, count: 0, swc: "3.0 - 3.2" }
      }
    },
    "Pulp & Paper": {
      selected: false,
      subIndustries: {
        "Integrated Mills": { selected: false, count: 0, swc: "30.5 - 33.0" },
        "RCF Mills": { selected: false, count: 0, swc: "9.9 - 13.0" }
      }
    },
    "Textiles": {
      selected: false,
      subIndustries: {
        "Integrated Industry (cotton)": { selected: false, count: 0, swc: "104.1 - 343.5" },
        "Fabric Processing Industry (cotton)": { selected: false, count: 0, swc: "51.1 - 97.5" },
        "Integrated Industry (woollen)": { selected: false, count: 0, swc: "237.1" }
      }
    },
    "Iron & Steel": {
      selected: false,
      subIndustries: {
        "Iron & Steel": { selected: false, count: 0, swc: "5.3 - 7.7" }
      }
    }
  });
  ...
  // Industrial Actions
  const toggleIndustryType = (industryType: string) => {
    setIndustrialSelection(prev => ({
      ...prev,
      [industryType]: {
        ...prev[industryType],
        selected: !prev[industryType].selected
      }
    }));
  };

  const toggleSubIndustry = (industryType: string, subIndustryName: string) => {
    setIndustrialSelection(prev => ({
      ...prev,
      [industryType]: {
        ...prev[industryType],
        subIndustries: {
          ...prev[industryType].subIndustries,
          [subIndustryName]: {
            ...prev[industryType].subIndustries[subIndustryName],
            selected: !prev[industryType].subIndustries[subIndustryName].selected
          }
        }
      }
    }));
  ...
  const updateSubIndustryCount = (industryType: string, subIndustryName: string, count: number) => {
    setIndustrialSelection(prev => ({
      ...prev,
      [industryType]: {
        ...prev[industryType],
  ...
  const canComputeIndustrialDemand = (): boolean => {
    if (!industrialChecked || selectedSubDistricts.length === 0) return false;
    
    // Check if at least one industry type is selected with at least one sub-industry
    const hasValidSelection = Object.keys(industrialSelection).some(industryType => {
      const industry = industrialSelection[industryType];
      if (!industry.selected) return false;
      
      return Object.keys(industry.subIndustries).some(subName => 
  ...
  const computeIndustrialDemand = async () => {
    try {
      ...
      // Prepare industrial data for API - include SWC values
      const industrialData: any = {};
      Object.keys(industrialSelection).forEach(industryType => {
        const industry = industrialSelection[industryType];
        if (industry.selected) {
          const selectedSubIndustries: any = {};
          Object.keys(industry.subIndustries).forEach(subName => {
            const subIndustry = industry.subIndustries[subName];
            if (subIndustry.selected) {
              selectedSubIndustries[subName] = {
                count: subIndustry.count,
                swc: subIndustry.swc  // Include SWC value range
              };
            }
          });
          
          if (Object.keys(selectedSubIndustries).length > 0) {
            industrialData[industryType] = selectedSubIndustries;
          }
        }
      });
