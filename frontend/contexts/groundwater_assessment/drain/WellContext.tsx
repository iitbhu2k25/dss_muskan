...
interface WellContextType {
  ...
  // Fetch wells data
  fetchWellsData: (selectedVillageCodes: number[]) => Promise<void>;
}
...
export const WellProvider: React.FC<WellProviderProps> = ({
  ...
  const fetchWellsData = async (selectedVillageCodes: number[]) => {
    setWellsLoading(true);
    setWellsError(null);
    try {
      const response = await fetch('/fastm/gwa/wells', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(selectedVillageCodes),
      });
...
useEffect(() => {
  ...
  const validateAndProcessCSV = async (file: File): Promise<{ success: boolean; message: string; data?: WellData[] }> => {
    try {
      ...
      const validateResponse = await fetch("/fastm/gwa/validate-csv", {
        method: "POST",
        body: formData,
      });
  ...
  const postCSVToBackend = async (csvData: string, filename: string): Promise<boolean> => {
    try {
      ...
      const response = await fetch("/fastm/gwa/upload-csv", {
        method: "POST",
        body: formData,
      });
  ...
  const confirmWellSelections = async (
    ...
    if (selectedVillageCodes.length === 0) {
      console.log("Cannot confirm: No villages selected");
      return null;
    }

    const selectedVillageObjects = villages.filter((v: any) =>
      selectedVillageCodes.includes(Number(v.village_code ?? v.id ?? v.code))
    );
