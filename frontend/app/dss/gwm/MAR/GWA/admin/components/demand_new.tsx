...
//   const [showIndustrialSelectionWarning, setShowIndustrialSelectionWarning] = useState(false);
...
//   // Handle Industrial Compute with Validation
//   const handleIndustrialClick = () => {
//     const isAnyIndustrySelected = Object.keys(industrialSelection).some(industryType => 
//       industrialSelection[industryType].selected
//     );
//     const isAnySubIndustrySelected = Object.keys(industrialSelection).some(industryType => {
//       const industry = industrialSelection[industryType];
//       return industry.selected && Object.keys(industry.subIndustries).some(subName => 
//         industry.subIndustries[subName].selected
//       );
//     });
    
//     if (!isAnyIndustrySelected || !isAnySubIndustrySelected) {
//       setShowIndustrialSelectionWarning(true);
//       return;
//     }
//     setShowIndustrialSelectionWarning(false);
//     computeIndustrialDemand();
//   };
...
//           <div className="mb-4">
//             <label className="block text-sm font-medium text-gray-700 mb-2">Select Industry Types:</label>
//             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//               {Object.keys(industrialSelection).map((industryType) => (
//                 <label key={industryType} className="flex items-center p-2 border border-gray-200 rounded-md hover:bg-gray-50 cursor-pointer">
//                   <input
//                     type="checkbox"
//                     checked={industrialSelection[industryType]?.selected || false}
//                     onChange={() => toggleIndustryType(industryType)}
//                     className="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded"
//                   />
//                   <span className="ml-2 text-sm text-gray-700 font-medium">{industryType}</span>
//                 </label>
//               ))}
//             </div>
//           </div>

//           {/* 4-Column Grid for Industry Sub-types */}
//           {Object.keys(industrialSelection).some(key => industrialSelection[key].selected) && (
//             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
//               {Object.keys(industrialSelection).map((industryType) => {
//                 const industry = industrialSelection[industryType];
//                 if (!industry.selected) return null;

//                 // Determine unit based on industry type
//                 const industryUnit = industryType === "Thermal Power Plants" ? "m³/MW" : "m³/MT";
                
//                 const subIndustries = industry.subIndustries;
//                 const selectedCount = Object.keys(subIndustries).filter(
//                   subName => subIndustries[subName].selected
//                 ).length;

//                 return (
//                   <div key={industryType} className="p-4 bg-indigo-50 border border-indigo-200 rounded-lg shadow-sm">
//                     <div className="mb-3">
//                       <h5 className="text-sm font-semibold text-indigo-800 mb-1">
//                         {industryType}
//                       </h5>
//                       <p className="text-xs text-gray-600">{industryUnit}</p>
//                     </div>

//                     <div className="space-y-3">
//                       {Object.keys(subIndustries).map((subIndustryName) => {
//                         const subIndustry = subIndustries[subIndustryName];
//                         const isSelected = subIndustry.selected || false;
//                         const count = subIndustry.count || 0;
//                         const swc = subIndustry.swc || "N/A";

//                         return (
//                           <div key={subIndustryName} className="border-b border-indigo-100 pb-2">
//                             <label className="flex items-start text-sm mb-2">
//                               <input
//                                 type="checkbox"
//                                 checked={isSelected}
//                                 onChange={() => toggleSubIndustry(industryType, subIndustryName)}
//                                 className="h-3 w-3 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded mr-2 mt-0.5 flex-shrink-0"
//                               />
//                               <span className="text-gray-700 leading-tight">{subIndustryName}</span>
//                             </label>
...
//                                   min="0"
//                                   value={count}
//                                   onChange={(e) => updateSubIndustryCount(industryType, subIndustryName, parseInt(e.target.value) || 0)}
//                                   className="w-full p-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
...
//                                 SWC: <span className="font-medium text-indigo-700">{swc}</span>
//                               </span>
//                             </div>
//                           </div>
//                         );
...
//                           ✓ Selected: {selectedCount} sub-industr{selectedCount !== 1 ? "ies" : "y"}
//                         </div>
//                       )}
//                     </div>
//                   </div>
...
//           {showIndustrialSelectionWarning && (
//             <div className="my-4 p-3 bg-yellow-100 border border-yellow-400 text-yellow-800 rounded-md flex items-center gap-2">
//               <svg className="w-5 h-5 text-yellow-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
//                 <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 3.011-1.742 3.011H4.42c-1.53 0-2.493-1.677-1.743-3.011l5.58-9.92zM10 13a1 1 0 100-2 1 1 0 000 2zm-1-4a1 1 0 011-1h.01a1 1 0 110 2H10a1 1 0 01-1-1z" clipRule="evenodd" />
//               </svg>
//               <span className="text-sm font-medium">Please select at least one industry type and one sub-industry.</span>
//             </div>
//           )}
