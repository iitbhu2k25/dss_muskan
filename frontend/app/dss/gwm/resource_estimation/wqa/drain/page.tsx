...
function GroundwaterAssessmentContent() {
  ...
  }, [activeStep]); // Only depend on activeStep, not removeAllRasterLayers
  ...
  const handleGeoJsonData = (data: { type: 'contour' | 'raster' | 'unified'; payload: any }) => {
    setContourData(data);
    if (data.type === 'raster') {
...
function GroundwaterContourWrapper({ activeStep }: { activeStep: number }) {
  ...
  return <GroundwaterContour activeTab="groundwater-contour" step={activeStep} />;
}
